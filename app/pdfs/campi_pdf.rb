class CampiPdf < Prawn::Document
    def initialize(relatorio, current_user, usuario_matricula)
      ...
      @relatorio.each do |relatorio|
        if(relatorio.matricula_aluno == @usuario_matricula)
          ...
          %i[center].each do |position|
            draw_text "Atenciosamente,", at: [200,190], :inline_format => true
            move_down 25
      ...
      if relatorio.avaliador == "Edina Lacerda"
          ...
          text "O <b>INSTITUTO FEDERAL FLUMINENSE</b>, CNPJ/MF nº 10.779.511/0001-07, situado à Rua Coronel Walter Kramer, nº 357, bairro: Parque Santo Antônio, município: Campos dos Goytacazes/RJ, CEP: 28080-565, neste Ato representado pela <b>Diretoria de Ensino dos Cursos Superiores de Licenciatura do <i>CAMPUS</i> CAMPOS CENTRO - INSTITUIÇÃO PROMOTORA -</b>, inscrito no CNPJ/MF sob o nº 10.779.511/0002-98, situada na Rua Doutor Siqueira, nº 273, Bairro: Parque Dom Bosco, município: Campos dos Goytacazes/RJ, CEP: 28.030-130, telefone (22) 27262897, <b>firma parceria</b>, em atendimento à Lei Nº 11.788, de 25/09/2008, com o <b>IFFluminense <i>campus</i> <u>#{relatorio.instituicao_apresentacao}</u> - Instituição CONCEDENTE -</b> inscrito no CNPJ/MF sob o nº <u>#{relatorio.cnpj}</u>, situada no(a) <u>#{relatorio.endereco_da_instituicao}</u>, Bairro <u>#{relatorio.bairro_da_instituicao}</u>, no munícipio de <u>#{relatorio.municipio_da_instituicao}</u>, Estado do <u>#{relatorio.estado_da_instituicao}</u>, CEP: <u>#{relatorio.cep_da_instituicao}<u>, telefone <u>#{relatorio.telefone_da_instituicao}</u>, representada por <u><b>#{relatorio.representante_da_instituicao}</u>, para fins de abertura de campo de Estágio Curricular Supervisionado dos Cursos de Licenciatura</b>, ao(à) <b>LICENCIANDO(A)</b>", align: :justify, :inline_format => true, :leading => 10
          move_down 3
          text "<b><u>#{relatorio.aluno_apresentacao}</u></b>", align: :center, size: 20, :inline_format => true, :leading => 10
          move_down 10
          text "matriculado(a) no Campus Campos Centro do IFFluminense, sob o nº <u>#{relatorio.matricula_aluno}</u>, no <u>#{relatorio.aluno_semestre}</u> semestre do ano letivo de <u>#{relatorio.ano}</u>, no Curso Superior de Licenciatura em <u>#{relatorio.licenciatura}</u>, <u>#{relatorio.periodo}</u> período. Residente no(a) <u>#{relatorio.endereco}</u>, nº <u>#{relatorio.numero}</u>, Complemento: <u>#{relatorio.complemento}</u>, Bairro: <u>#{relatorio.bairro}</u>, município: <u>#{relatorio.municipio}</u>, UF: <u>#{relatorio.estado}</u>, CEP: <u>#{relatorio.cep}</u>, telefone/contato: nº <u>#{relatorio.telefone}</u>", align: :justify, :inline_format => true, :leading => 10
          move_down 10
