class CentroPdf < Prawn::Document
    def initialize(relatorio, current_user, usuario_matricula)
      ...
      @usuario_matricula = usuario_matricula
        ...
        move_down 20
        text "Campos dos Goytavazes, RJ, ... de ............. de 20.....",align: :right
        move_down 50
        text "CARTA DE APRESENTAÇÃO", align: :center, size: 20
        move_down 20
        text "<b>DA:</b> Diretoria de Ensino Superior dos Cursos de Licenciatura/Núcleo de Apoio à Prática Profssional(NAPP) do IFFluminense <i>campus</i> Campos Centro.", align: :center, :inline_format => true, :leading => 10
        move_down 30
        text "<b>À:</b>Direção da Instituição de Ensino: #{current_user.nome_da_instituicao}", align: :left, size: 15, :inline_format => true, :leading => 10
        move_down 20
        text "<b>Assunto: <i>Apresentação de discente-estagiário</i></b>",align: :left, :inline_format => true, :leading => 10
        move_down 30
        ...
        text "Prezado Gestor", align: :justify, :inline_format => true, :leading => 10
            text "O IFFluminense <i>campus</i> Campos Centro apresenta o(a) licenciando(a)-estagiário(a) #{current_user.nome_social}, matriculado(a), neste instituto, no #{current_user.periodo} período do Curso Superior de Licenciatura em #{current_user.licenciatura}, no #{current_user.semestre_apresentacao }semestre letivo de #{current_user.ano_apresentacao}, para fins de cumprimento das atividades do Estágio Curricular Supervisionado no campo de estágio dessa Instituição Concedente, prescristas no Plano de Atividades de Estágio(PAE), conforme Calendário Acadêmico deste <i>Campus</i>, em atendimento a carga horária total destinada ao período em curso. Ressaltamos que o Estágio Curricular Supervisionado é componente obrigatório e de importância significativa para a formação dos futuros formadores.", align: :justify, :inline_format => true, :leading => 10
            
            text "Agradecemos a atenção sempre nos concedida e a seriedade com que os profissionais da educação têm acolhido e acompanhado nossos(as) licenciados(as). Sem mais, colocamo-nos à disposição para quaisquer esclarecimentos e registramos, nesta, os contatos com DIRLIC/NAPP. (22) 2726-2897 // dirlicenciatura.camposcentro@iff.edu.br, bem como o site licenciaturas.centro.iff.edu.", align: :justify, :inline_format => true, :leading => 10
