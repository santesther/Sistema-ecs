    ...
    def initialize(relatorio, current_user, usuario_matricula)
      ...
      if relatorio.avaliador == "Edina Lacerda"
          ...
          text "Usuário não cadastrado"
            ...
            text "<b>Apólice de seguro nº: </b><u>#{relatorio.apolice}</u> ", align: :center, size: 12, :inline_format => true, :leading => 10
            move_down 10
            text "O <b>INSTITUTO FEDERAL FLUMINENSE</b>, CNPJ/MF nº 10.779.511/0001-07, situado à Rua Coronel Walter Kramer, nº 357, bairro: Parque Santo Antônio, município: Campos dos Goytacazes/RJ, CEP: 28080-565, neste Ato representado pela <b>Diretoria de Ensino dos Cursos Superiores de Licenciatura do CAMPUS CAMPOS CENTRO - INSTITUIÇÃO PROMOTORA -</b>, inscrito no CNPJ/MF sob o nº 10.779.511/0002-98, situada na Rua Doutor Siqueira, nº 273, Bairro: Parque Dom Bosco, município: Campos dos Goytacazes/RJ, CEP: 28.030-130, telefone (22) 27262897, <b>firma parceria</b>, em atendimento à Lei Nº 11.788, de 25/09/2008, com a <b>Instituição de Ensino CONCEDENTE - <u>#{relatorio.instituicao_apresentacao}</u> -</b> inscrita no CNPJ/MF sob o nº <u>#{relatorio.cnpj}</u>, situado no(a) <u>#{relatorio.endereco_da_instituicao}</u>, Bairro: <u>#{relatorio.bairro_da_instituicao}</u>, no município de <u>#{relatorio.municipio_da_instituicao}</u>, Estado do <u>#{relatorio.estado}</u> , CEP: <u>#{relatorio.cep_da_instituicao}</u>, telefone <u>#{relatorio.telefone_da_instituicao}</u>, representada por <b><u>#{relatorio.representante_da_instituicao}</u>, para fins de continuidade do Estágio Curricular Supervisionado dos Cursos de Licenciatura</b>, ao(à) <b>LICENCIANDO(A)</b>", align: :justify, :inline_format => true, :leading => 10
            move_down 10
